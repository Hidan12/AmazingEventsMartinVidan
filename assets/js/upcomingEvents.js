...
function card(idHtml, obj){
  idHtml.innerHTML += `
  <div class="card col-sm-8 col-md-5 col-xl-3">
      <img src=${obj.image} class="card-img-top object-fit-cover p-2 heigth-img" alt="...">
...
function addInfo(info){
    let datosArray = info.events
    let fechaPresente =  new Date(info.currentDate)
    let cards = document.getElementById("cards")
    let categoryHtml = document.getElementById("category")
    let category = []
  
    for (let i = 0; i < datosArray.length; i++) {
        let fechaItems = new Date(datosArray[i].date)
        if(fechaItems > fechaPresente){
            card(cards, datosArray[i])
        }
        if(!category.includes(datosArray[i].category)){
            category.push(datosArray[i].category)
